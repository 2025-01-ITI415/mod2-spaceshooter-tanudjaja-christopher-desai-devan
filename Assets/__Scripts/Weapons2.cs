    ...
    private void Fire()
    {
        ...
        switch (type)
        {
            ...
            case eWeaponType.missile:
                p = MakeProjectile();
                p.vel = vel;
                StartCoroutine(HomeOnTarget(p));
                break;
        }
    }

    private IEnumerator HomeOnTarget(ProjectileHero missile)
    {
        while (missile != null)
        {
            ...
            {
                Vector3 direction = (nearestEnemy.transform.position - missile.transform.position).normalized;
                missile.vel = direction * def.velocity;
            }
            yield return null;
        }
    }
